...
支持截图、视频、截图加视频三种输入。新页面默认 HTML/CSS/JavaScript 与本地资源；已有项目沿用原框架。

这是供当前 Agent 使用的技能说明与本地辅助工具。画面理解、交互判断、页面代码编写和浏览器检查由 Agent 使用所在环境的工具完成；本仓库的 Python 脚本负责准备构建说明、裁切素材、抽帧、检查证据引用及生成对比产物，不直接调用模型生成页面，也不自带浏览器驱动。

## 安装

`main` 包含截图复刻流程及视频交互、动画扩展。纯截图任务沿用原流程，无需提供视频或交互说明文件。

```bash
git clone --branch main https://github.com/hxx2001/screenshot_to_code_agent-skill.git \
...
已通过 Git 安装的用户，先确认工作区没有需要保留的修改，再更新 `main`：
...
若安装目录没有 `.git`，应先在独立仓库副本中拉取 `main`，再同步技能文件；不要在旧版目录直接运行 `git pull`。同步时包含新增的 `references/` 与 `scripts/` 文件，不要只替换 `SKILL.md`，并先备份自己的修改。

## 当前能力

截图任务支持单张或多张图片，沿用上游截图提示词生成构建说明，并提供素材裁切、HTML 提取和等尺寸图片对比工具。

视频任务在同一页面复刻流程上增加以下工具和工作约定：
...
- 对计时、手势和滚动动画，校验器要求通过记录包含连续帧审阅范围、覆盖中间过程的本地对比报告，以及交互和动效审查文字；它能检查格式与引用，不能核实审查是否真实完成。
...
技能要求 Agent 在每次视频任务中根据该录屏编写组件和动画源码，分别放入生成项目的 `components/` 和 `animations/` 目录。已有项目沿用框架和目录约定，例如 `src/components/`、`src/animations/`。页面应实际引用这些文件，后续校准在同一组文件上迭代。这是生成流程要求；辅助脚本不会自动生成这些模块或检查页面是否已引用它们。
...
截图复刻是基础流程，视频能力在此基础上补充交互状态与动画还原。两种输入遵循相同的页面视觉检查要求；静态视觉、交互行为和动态表现分别记录检查结果。

页面使用可编辑的前端代码实现。照片、头像、插画、标志及含整体艺术字的海报等视觉素材可以复用或裁切；界面文案、数字、按钮、布局和交互状态保持代码实现，不使用整张截图或覆盖透明点击区域的截图代替页面。

视频工具处理本地媒体。用户提供的截图、录屏及生成页面放在任务输出目录，不提交到技能仓库。

逐帧变化扫描只定位画面变化候选片段，不自动识别按钮含义、触发动作或业务逻辑。状态和交互说明由 Agent 根据实际查看的画面与用户补充整理。未显示的触点、手势、未演示的路径及无法确定的缓动参数，应明确标注为用户描述、推断或未知；近似实现不能宣称精确还原。音频不在默认分析范围内。

本地页面中的关注、发布、购买等交互用于演示，不代表已接入真实业务服务或已执行真实业务操作。

素材清晰度、字体、录屏完整性和运行环境会影响还原效果。脚本测试通过、像素误差统计或生成对比图，都不等于页面或动画验收通过。实际效果仍需使用可用的浏览器工具检查、截图对比和交互回放；未检查的部分应标明未验证，证据或工具不足时应说明具体缺口，不给出未经验证的还原度百分比。
